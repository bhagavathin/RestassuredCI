...
public class DriverScript {
	...
	public static void main(String[] args) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, IOException, NoSuchMethodException, SecurityException {
		FileInputStream fs = new FileInputStream(System.getProperty("user.dir")+"//src//img//auto//config//config.properties");
		CONFIG= new Properties();
		CONFIG.load(fs);
		
		fs = new FileInputStream(System.getProperty("user.dir")+"//src//img//auto//config//or.properties");
		OR= new Properties();
		OR.load(fs);
	...
	public void start() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException{
		...
		suiteXLS = new Xls_Reader(System.getProperty("user.dir")+"//src//img//auto//xls//Suite.xlsx");
		
		System.out.println("Total Number of Suite      : " + suiteXLS.getRowCount(Constants.TEST_SUITE_SHEET));
		for(currentSuiteID=2;currentSuiteID<=suiteXLS.getRowCount(Constants.TEST_SUITE_SHEET);currentSuiteID++){
			...
			if(suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.RUNMODE, currentSuiteID).equals(Constants.RUNMODE_YES)){
				...
				System.out.println("******Executing the Suite******"+suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.Test_Suite_ID, currentSuiteID));
				currentTestSuiteXLS=new Xls_Reader(System.getProperty("user.dir")+"//src//img//auto//xls//"+currentTestSuite+".xlsx");
